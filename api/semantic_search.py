...
class SemanticSearch:
    def __init__(self, model_name: str = "all-MiniLM-L6-v2", max_docs: int = 0, batch_size: int = 32, dtype: str = "float16"):
        """Initialize semantic search with sentence transformer model.

        Args:
            model_name: ST model name (use smaller on 512Mi, e.g. 'paraphrase-MiniLM-L3-v2').
            max_docs: Cap number of docs to embed (0 = no cap).
            batch_size: Encode batch size to control RAM.
            dtype: 'float16' (default) to save memory, or 'float32'.
        """
        ...
        self.max_docs = max(0, int(max_docs))
        self.batch_size = max(1, int(batch_size))
        self.dtype = np.float16 if str(dtype).lower() == "float16" else np.float32
    ...
    def create_embeddings(self, search_index):
        ...
        for doc in search_index:
            ...
            documents.append({'id': len(documents), 'text': text, 'doc': doc})
            count += 1
            if self.max_docs and count >= self.max_docs:
                break

        # Generate embeddings with small batches and float16 to save RAM
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            batches.append(enc.astype(self.dtype, copy=False))
        self.embeddings = np.vstack(batches) if batches else np.empty((0, 384), dtype=self.dtype)
        ...
        logger.info(f"Created and saved embeddings for {len(documents)} documents (dtype={self.embeddings.dtype}, batch_size={self.batch_size})")
    ...
    def search(self, query, top_k=10):
        ...
        # Encode query
        query_embedding = self.model.encode([query], convert_to_numpy=True, normalize_embeddings=True).astype(self.embeddings.dtype, copy=False)
